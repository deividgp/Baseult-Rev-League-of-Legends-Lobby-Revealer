...
public partial class Form1 : Form
{
    ...
    private async void Button1_Click(object sender, EventArgs e)
    {
        ...
        GetData(await MakeRequest(HttpMethod.Get, "/chat/v5/participants/champ-select", false));
    }
    ...
    private async void Button3_Click(object sender, EventArgs e)
    {
        ...
        if (dialogResult == DialogResult.Yes)
        {
            ...
            await MakeRequest(HttpMethod.Post, "/lol-login/v1/session/invoke?destination=lcdsServiceProxy&method=call&args=[\"\",\"teambuilder-draft\",\"quitV2\",\"\"]", true);
    ...
    private async void GetData(string req)
    {
        Players.Clear();

        string uggPlayers = string.Empty;
        string region = (string)JObject.Parse(await MakeRequest(HttpMethod.Get, "/riotclient/get_region_locale", true))["region"];
    ...
    private async Task<string> MakeRequest(HttpMethod type, string url, bool isClient)
    {
        try
        {
            ...
            HttpRequestMessage request = new(type, new Uri("https://127.0.0.1:" + port + url));
